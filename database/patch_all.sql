...
CREATE TABLE IF NOT EXISTS connect_bundle_options (
    ...
    sort_order      INTEGER NOT NULL DEFAULT 0,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT connect_bundle_options_amount_positive CHECK (connect_amount > 0),
    CONSTRAINT connect_bundle_options_price_positive CHECK (price_cents > 0)
);
...
CREATE INDEX IF NOT EXISTS idx_connect_checkouts_user_id ON connect_checkouts (user_id);
CREATE INDEX IF NOT EXISTS idx_connect_checkouts_status ON connect_checkouts (user_id, status);

CREATE UNIQUE INDEX IF NOT EXISTS idx_connect_checkouts_user_pending_bundle_promo
    ON connect_checkouts (user_id, bundle_option_id, COALESCE(LOWER(TRIM(promo_code)), ''))
...
-- ---------------------------------------------------------------------------
-- 12. withdrawal_methods (talent payouts)
-- ---------------------------------------------------------------------------
ALTER TABLE users
